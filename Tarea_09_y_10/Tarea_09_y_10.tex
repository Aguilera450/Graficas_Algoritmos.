...
\item (2 puntos) Sea $T$ un \'arbol de DFS de una gr\'afica conexa no
  ...
  \begin{proof} 
    ...
    Como para cualesquiera tres vértices de un bloque de $G$ existe una trayectoria que una a cualesquiera dos de
    ellos y que no pasa por el tercero, entonces siempre podemos garantizar que el siguiente vértice que visitemos
    después de $w$ no saldrá de $v$. Además DFS ya no agrega a la pila los vértices previamente visitados. Así, en
    el árbol DFS de $G$, $G \bigcap T$ sólo tendrá los vértices de $T$ y ahí la raíz tendrá $d(v)=1$ por lo mencionado
    anteriormente. 
