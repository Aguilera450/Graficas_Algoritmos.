...
\begin{algorithm}[H]
  ...
  \KwIn{Una gr\'afica $H$.}
  \KwOut{\code{TRUE} o \code{FALSE} dependiendo si la entrada contiene un ciclo hamiltoniano.}
  \BlankLine {
%    \For{$v \in V_D$ \textnormal{\textbf{and}} $d^-(v) = 0$}{
%      $v \gets$ 0\;
%    }
...
%      }
             ...
             %{\Return $D$}
             \caption{HamiltonCycle$(H;\code{true}/\code{false})$} \label{HamiltonCycle}%\textnormal{new}
             \DecMargin{1em}
\end{algorithm}
