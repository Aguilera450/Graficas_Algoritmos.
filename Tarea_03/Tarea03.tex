...
\item Una gr\'afica es \textit{escindible completa} si su conjunto de
  ...
  \begin{proof}
    ...
    \item[$\Leftarrow$)] Para este caso analicemos a todas las gr\'aficas no isomorfas
      ...
      Notar que $H_2, H_3, H_4, H_5, H_6$, y $H_8$ contienen como subgr\'aficas inducidas a
      $\overline{P_3}$, luego s\'olo $H_1, H_7, H_9$, y $H_{10}$ junto a $G_1, G_2$, y $G_3$
      ...
      ambos subconjuntos de $V_G$, con esto tenemos que $G$ es \textit{escindible}. Por
      ($2$), ($4$) y ($5$) vemos que es necesario que haya aristas entre v\'ertices de $S$
      y $K$, como ($1$) y ($3$) no restringen la condición anterior, entonces se puede
      considerar a \textit{escindible completa}.
    \end{itemize}
    
    De los casos anterior concluimos que una gr\'afica es escindible completa si y s\'olo si
