      ...
      \begin{proof}
        ...
        \begin{itemize}
          \item \textbf{Caso 1)} $a_{rj} = 0$.
            ...
            Si $a_{rj} = 0$, entonces $(t_{ir} \cdot a_{rj} = 0)$ y por tanto, existen $0$ caminos de longitud $n+1$.

          \item \textbf{Caso 2)} $a_{rj} = 1$.

            Si $a_{rj} = 1$, entonces $(t_{ir} \cdot a_{rj} = t_{ir})$ y por tanto, existe $1$ camino de longitud
            $n+1$ que recorre de $v_i v_j$.

            De lo anterior se sigue que (independientemente de si $a_{rj} = 0$ o $a_{rj} = 1$)
            $\displaystyle \sum_{r=1}^{n} t_{ir} a_{rj}$ nos dará el número de caminos de longitud $n+1$
            que existen entre $v_i v_j$.
        \end{itemize}
      \end{proof}
